-- written by groverbuger for blazeinc
-- september 2021
-- MIT license

local vectors = require(blazeinc.path .. "/vectors")
...
function collisions.hasLevelCollided(sizeof, x,y,z)
    local floor = sizeof
    if blazeinc.world.getCameraX() > x
    and blazeinc.world.getCameraX() < x+floor then
        return true
    elseif blazeinc.world.getCameraY() > y
    and blazeinc.world.getCameraY() < y+floor then
        return true
    elseif blazeinc.world.getCameraZ() > z
    and blazeinc.world.getCameraZ() < z+floor then
        return true
    else
        return false
    end
...
function collisions.hasCameraCollided(length, x,y,z)
    local floor = length
    if blazeinc.world.getCameraX() >= x-floor and
    blazeinc.world.getCameraX() <= x+floor and
    blazeinc.world.getCameraY() >= y-floor and
    blazeinc.world.getCameraY() <= y+floor and
    blazeinc.world.getCameraZ() >= z-floor and
    blazeinc.world.getCameraZ() <= z+floor
    then
        return true
    else
        return false
